"""driftgate.yaml -- the project config."""
...
DEFAULT_CONFIG = Path("driftgate.yaml")

TEMPLATE = """\
# driftgate -- security regression gate for LLM agents
#
# Workflow:
#   driftgate baseline    # record current posture, then COMMIT the baseline file
#   driftgate check       # re-run and fail if anything got worse

# WHAT TO TEST. Pick one of the two:
#
# (a) models -- driftgate drives its own agent loop against each model.
...
models:
  ...
  # - provider: litellm                    # pip install 'driftgate[litellm]'
...
# `driftgate roles` to see the mapping and override anything wrong here.
# tool_roles:
...
# Built-in attack packs. `driftgate packs` lists them.
packs:
  - prompt_injection
  - tool_abuse
  - data_exfiltration
...
# `driftgate check` will tell you when a case is under-powered.
samples: 20
concurrency: 4
...
class Config:
    ...
    def load(cls, path: Path = DEFAULT_CONFIG) -> Config:
        if not path.exists():
            raise FileNotFoundError(
                f"{path} not found -- run `driftgate init` to create one"
            )
        ...
        return cls(
            ...
            cache_dir=str((data.get("cache") or {}).get("dir", ".driftgate-cache")),
    ...
    def build_targets(
        ...
        is a strictly better test subject than driftgate's generic loop.
        """
        ...
        if not specs:
            raise ValueError(
                "nothing to test: set `models:` or `target:` in driftgate.yaml, "
                "or pass --model"
            )
    ...
    def build_target(self, model_override: list[str] | None = None, cache: Any = None) -> Any:
        ...
        if len(targets) > 1:
            raise ValueError(
                ...
                f"Use --model to pick, or `driftgate compare` for a matrix."
            )
        return targets[0]
