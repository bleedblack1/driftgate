"""driftgate CLI: init / baseline / check / packs / demo."""
...
def _make_app() -> typer.Typer:
    return typer.Typer(
        ...
        help=(
            "Detect security regressions in LLM agents when the model, prompt, "
            "or tools change."
        ),
    )
...
@app.command()
def init() -> None:
    """Create driftgate.yaml in the current directory."""
    ...
    console.print("Next: point `target:` at your agent, then run `driftgate baseline`.")
...
def baseline(
    ...
    console.print("\n[bold]Commit this file.[/] It is the record `driftgate check` compares against.")
...
def check(
    ...
    if not baseline_path.exists():
        console.print(f"[red]no baseline at {baseline_path}[/] -- run `driftgate baseline` first")
        raise typer.Exit(2)
    ...
    if blocked and not allow_skipped:
        ...
        console.print(
            "[yellow]Fix the tool-role mapping (`driftgate roles`), or pass "
...
def scan(
    ...
    """Run the corpus against a model once. No baseline, no config needed.
        ...
        driftgate scan --model ollama:llama3.1
    ...
    target = cfg.build_target(list(model or []) or None, cache=cache)
    ...
    _cache_line(cache, console)
    console.print("\n[dim]Snapshot only. `driftgate baseline` to start gating changes.[/]")
...
def compare(
    ...
    cases = corpus_mod.load(cfg.packs or None, [Path(d) for d in cfg.corpus_dirs])
    cache = cfg.make_cache()
    targets = cfg.build_targets(list(model or []) or None, cache=cache)
    if len(targets) < 2:
...
def roles(
    ...
    here should be corrected with `tool_roles:` in driftgate.yaml.
    """
    from rich.table import Table
    ...
    cfg = Config.load(config) if config.exists() else Config()
    if model or cfg.target:
        # A configured target may wrap tools we cannot see any other way.
        target = cfg.build_target(list(model or []) or None)
        rm = getattr(target, "role_map", None)
        if rm is None:
            console.print("[yellow]this target does not expose a role map[/]")
            raise typer.Exit(1)
        skip_for = target.skip_reason
    else:
        # No model needed: roles are a property of the toolkit alone.
        tools, _prompt = cfg.resolve_toolkit()
        rm = roles_mod.build(tools, cfg.tool_roles or None)
        from .adapters.inprocess import skip_reason as _skip

        def skip_for(case):
            return _skip(case, rm)
    ...
    cases = corpus_mod.load(cfg.packs or None, [Path(d) for d in cfg.corpus_dirs])
    blocked = [(c.id, skip_for(c)) for c in cases]
    ...
    if blocked:
        ...
        console.print(
            "\n[yellow]Add the missing roles under `tool_roles:` in driftgate.yaml.[/]"
        )
    else:
...
def ui(
    ...
    console.print(f"[bold]driftgate ui[/] -> [underline]{url}[/]")
...
def main() -> None:
    """Entry point. Configuration problems are messages, not stack traces:
    a first run that ends in a traceback reads as a broken tool even when the
    underlying message is the right one."""
    try:
        sys.exit(app())
