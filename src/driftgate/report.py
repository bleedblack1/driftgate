...
def to_terminal(report: DiffReport, console: Console | None = None) -> None:
    ...
    order = {"regressed": 0, "skipped": 1, "new": 2, "improved": 3, "unchanged": 4, "removed": 5}
    for c in sorted(report.cases, key=lambda c: (order[c.verdict], c.case_id)):
        if c.verdict == "unchanged":
            continue
        if c.verdict not in order:
            continue
        table.add_row(
            c.case_id,
            c.severity,
...
def to_markdown(report: DiffReport) -> str:
    ...
    lines: list[str] = []
    if report.passed:
        lines.append("### driftgate: no security or utility regressions")
    else:
        sec, util = len(report.security_regressions), len(report.utility_regressions)
        parts = [f"{n} {k}" for n, k in ((sec, "security"), (util, "utility")) if n]
        lines.append(f"### driftgate: {' + '.join(parts)} regression(s)")
...
def metrics_panel(m, console: Console | None = None, *, title: str = "metrics") -> None:
    ...
    if m.skipped:
        ...
        console.print(
            "  [yellow]Map your tools to the missing roles in driftgate.yaml "
            "(`driftgate roles`), or these risks go unchecked.[/]\n"
        )
    console.print(head)
