...
driftguard must not care who serves the model. Two backends cover essentially
...
class ChatModel(Protocol):
    """Implement this to support a provider driftguard has never heard of."""
...
class LiteLLMModel:
    ...
    async def chat(
        ...
        try:
            ...
            raise RuntimeError(
                "provider 'litellm' requires the extra: pip install 'driftguard[litellm]'"
            ) from exc
...
class EchoModel:
    """Offline stand-in for tests and `driftguard demo`. Never calls out."""
...
def from_config(spec: dict[str, Any] | str) -> ChatModel:
    """Build a ChatModel from driftguard.yaml.
